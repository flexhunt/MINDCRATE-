-- Fix the dashboard stats function with proper column references
...
DECLARE
    ...
    user_current_streak INTEGER DEFAULT 0;
...
BEGIN
    ...
    -- Get total quiz score - SUM of all scores from user_progress (same as leaderboard)
    ...
    -- Get current streak from daily_rewards (use alias to avoid ambiguity)
    SELECT COALESCE(MAX(dr.current_streak), 0) INTO user_current_streak
    ...
    SELECT NOT EXISTS (
        ...
        AND dr2.created_at::date = CURRENT_DATE
